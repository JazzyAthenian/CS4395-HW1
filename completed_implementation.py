...
def compute_unigram_add_k(fileName, k):
    ...
    #Calulating weights with k value
    for key in tokenDict:
...
def processing_test(bigram, fileName):
    # Checking if each token in the test file is part of the vocabulary
    f = open(fileName, "r")
    test_data = []
...
if __name__ == '__main__':
    ...
    unigram_add_k2 = compute_unigram_add_k(train_file, k=0.8)
    ...
    bigram_add_k2 = compute_bigram(train_file, smoothing="add_k", k=0.8)
    ...
    print(f"Unigram (Add-k, k=0.8): {compute_perplexity(unigram_add_k2, test_file, ngram='unigram', smoothing='add_k', k=0.8)}")
    ...
    print(f"Bigram (Add-k, k=0.8): {compute_perplexity(bigram_add_k2, test_file, ngram='bigram', smoothing='add_k', k=0.8)}")
